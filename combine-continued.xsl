...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:copy>
      ...
      <xsl:for-each select="node()">
         ...
         <!-- lb external of app: show only, if not part of rdg-group -->
         <xsl:when test="self::tei:lb[not(ancestor::tei:table)]">
            <xsl:if test="generate-id() = generate-id(key('lb-by-n', @n)[1])">
               <xsl:apply-templates select="." mode="continued"/>
            </xsl:if>
         </xsl:when>
   ...
   <xsl:template match="tei:lb[not(ancestor::tei:table)]" mode="dedup-lb">      
      <xsl:choose>
   ...
   <!-- No white space or lines -->
   <xsl:template match="text()" mode="dedup-lb">
      <xsl:value-of select="
         replace(
            replace(., '(^[\r\n\t ]+)|([\r\n\t ]+$)', ' '),     (: replace outer whitespace with space :)
            '\s+',                                              (: multiple spaces/breaks :)
            ' '                                                 (: normalise to simple space :)
         )
      "/>
   </xsl:template>

<!-- for tables: line break before each row element -->
<xsl:template match="tei:row" mode="dedup-lb">
   <xsl:text>
         </xsl:text>
  <xsl:copy>
    <xsl:apply-templates select="@* | node()" mode="dedup-lb"/>
...
<!-- for tables: line break before each cell element -->
<xsl:template match="tei:cell" mode="dedup-lb">
      <xsl:text>
            </xsl:text>
  <xsl:copy>
    <xsl:apply-templates select="@* | node()" mode="dedup-lb"/>
...
<!-- remove leading whitespace in tables -->
<xsl:template match="text()[ancestor::tei:table]" mode="dedup-lb">
  <xsl:value-of select="replace(., '^[\r\n\t ]+', '')"/>
</xsl:template>
